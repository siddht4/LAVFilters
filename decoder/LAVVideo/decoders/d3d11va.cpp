...
STDMETHODIMP CDecD3D11::PostConnect(IPin *pPin)
{
  ...
  ID3DVideoMemoryConfiguration *pD3DVidMemConfig = nullptr;
  hr = pPin->QueryInterface(&pD3DVidMemConfig);
  if (FAILED(hr)) {
    DbgLog((LOG_ERROR, 10, L"-> ID3DVideoMemoryConfiguration not available, using fallback mode"));
  }
  ...
  UINT nDevice = pD3DVidMemConfig ? pD3DVidMemConfig->GetD3D11AdapterIndex() : 0;
  char deviceId[34] = { 0 };
  ...
  if (ret < 0) {
    ...
    SafeRelease(&pD3DVidMemConfig);
    return E_FAIL;
  }

  // TODO: verify hardware support

  // Notice the connected pin that we're sending D3D11 textures
  if (pD3DVidMemConfig)
  {
    hr = pD3DVidMemConfig->SetD3DSurfaceType(D3D_SurfaceType_D3D11Texture);
    SafeRelease(&pD3DVidMemConfig);
...
STDMETHODIMP CDecD3D11::AllocateFramesContext(int width, int height, AVPixelFormat format, int nSurfaces, AVBufferRef **ppFramesCtx)
{
  ...
  *ppFramesCtx = av_hwframe_ctx_alloc(m_pDevCtx);
  ...
  pFramesHWContext->BindFlags |= D3D11_BIND_DECODER;
